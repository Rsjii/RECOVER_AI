 ...
 */
export async function clearOldNotifications(companyId: string, daysOld: number = 90): Promise<number> {
  try {
    const result = await pool.query<{ count: string }>(
      `DELETE FROM notification_events
       WHERE company_id = $1 AND created_at < NOW() - INTERVAL '${daysOld} days'
       RETURNING COUNT(*) as count`,
      [companyId]
    );
