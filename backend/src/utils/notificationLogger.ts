import { createNotificationEvent } from '../db/notificationEvents';
...
/**
 * Log when daily actions are available
 */
export async function logDailyActionsAvailableNotification(
  ...
  try {
    await createNotificationEvent(
      companyId,
      'actions_available',
      `✅ ${actionCount} recommended actions available today`,
      {
        message: `Your agent has identified ${actionCount} recommended actions for today. Review and approve.`,
        icon: '✅',
        priority: 'warning',
        action_url: '/dashboard',
        action_label: 'Review Actions',
        metadata: {
          event_type: 'daily_actions',
          action_count: actionCount,
        },
      }
    );
  } catch (err) {
    console.error('Error logging daily actions notification:', err);
  }
}
