...
export async function checkAndNotifySystemHealth(companyId: string): Promise<void> {
  ...
  try {
    ...
    // Check Email configuration (check if SMTP is verified or set up)
    const hasEmailConfig = await pool.query(
      `SELECT EXISTS(SELECT 1 FROM companies WHERE id = $1 AND (smtp_host IS NOT NULL OR mail_from_email IS NOT NULL))`,
      [companyId]
    );
