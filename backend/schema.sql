...
-- ============================================================
-- TEMPORARY_OTPS (Email Verification - Signup Flow)
-- Stores OTP codes temporarily during signup process
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_temporary_otps_expires ON temporary_otps(expires_at);


-- ============================================================
-- PAYABLES (CashOS - Bills/Expenses Tracking)
-- ============================================================
CREATE TABLE IF NOT EXISTS payables (
  ...
  category VARCHAR(50) DEFAULT 'other',  -- 'payroll', 'rent', 'cloud', 'marketing', 'other'
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_payables_due_date ON payables(company_id, due_date);
CREATE INDEX IF NOT EXISTS idx_payables_category ON payables(company_id, category);

-- ============================================================
-- FORECAST_ASSUMPTIONS (CashOS - User's Manual Forecast Inputs)
-- ============================================================
CREATE TABLE IF NOT EXISTS forecast_assumptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  growth_rate_pct NUMERIC(5, 2) DEFAULT 0,  -- e.g., 10.5 for 10.5% monthly growth
  payroll_amount_monthly NUMERIC(15, 2),  -- monthly payroll
  other_expenses_monthly NUMERIC(15, 2),  -- other monthly operating expenses
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT fk_assumptions_company FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE CASCADE,
  CONSTRAINT unique_assumptions_per_company UNIQUE (company_id)
);

CREATE INDEX IF NOT EXISTS idx_forecast_assumptions_company ON forecast_assumptions(company_id);
