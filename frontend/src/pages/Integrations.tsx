...
export default function Integrations() {
  ...
  // Auth + pipeline guard
  useEffect(() => {
    document.title = 'Connect Billing — CashOS';
    ...
    // Backend is source of truth — redirect if not at integrations stage
    const stage = company?.onboardingStage;
    if (stage) {
      if (stage === 'pending' || stage === 'details_form') {
        // User hasn't completed signup/OTP yet
      ...
      // On integrations stage — sync local state and stay here
      if (stage === 'integrations') {
        updateState({ accountCreated: true });
      }
    }
  }, [user, company?.onboardingStage]);
  ...
  const handleValidateStripeKey = async () => {
    ...
    try {
      ...
      await api.post('/api/audits/integrations/validate-stripe-key', {
  ...
  const handleCsvUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post('/api/audits/integrations/upload-invoices', {
  ...
  const handleProceedToAudit = async () => {
    ...
    try {
      ...
      await api.post('/api/audits/integrations/proceed');
      ...
      navigate('/generate-audit', { replace: true, state: { fromIntegrations: true } });
    } catch (err: any) {
