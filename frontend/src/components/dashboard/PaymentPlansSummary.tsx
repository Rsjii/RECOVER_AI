...
export default function PaymentPlansSummary({ summary, loading = false }: PaymentPlansSummaryProps) {
  ...
  return (
    <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/[0.06] rounded-xl p-5">
      <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Payment Plans</h3>
      ...
      {isEmpty ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div className="bg-gray-50 dark:bg-[#18181b] rounded-lg px-3 py-2">
              <p className="text-[10px] text-gray-500 dark:text-zinc-500 uppercase tracking-wide">Active</p>
              <p className="text-xl font-bold text-gray-900 dark:text-white">{summary?.activePlans ?? 0}</p>
            </div>
            <div className="bg-gray-50 dark:bg-[#18181b] rounded-lg px-3 py-2">
              <p className="text-[10px] text-gray-500 dark:text-zinc-500 uppercase tracking-wide">Acceptance</p>
              <p className={`text-xl font-bold ${acceptStatus}`}>{summary?.acceptanceRate ?? 0}%</p>
            </div>
            <div className="bg-gray-50 dark:bg-[#18181b] rounded-lg px-3 py-2">
              <p className="text-[10px] text-gray-500 dark:text-zinc-500 uppercase tracking-wide">Completion</p>
              <p className={`text-xl font-bold ${completeStatus}`}>{summary?.completionRate ?? 0}%</p>
            </div>
            <div className="bg-gray-50 dark:bg-[#18181b] rounded-lg px-3 py-2">
              <p className="text-[10px] text-gray-500 dark:text-zinc-500 uppercase tracking-wide">Active Value</p>
              <p className="text-xl font-bold text-gray-900 dark:text-white">{fmt(summary?.totalValueActive ?? 0)}</p>
            </div>
          </div>
