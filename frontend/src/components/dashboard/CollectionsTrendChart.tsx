...
export default function CollectionsTrendChart({ data, loading = false }: CollectionsTrendChartProps) {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/[0.06] rounded-xl p-5 animate-pulse h-56">
        ...
        <div className="h-36 bg-gray-100 dark:bg-white/5 rounded mt-4" />
      </div>
    );
  }
  ...
  if (!data.length) {
    return (
      <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/[0.06] rounded-xl p-5 flex items-center justify-center h-56">
        <p className="text-xs text-gray-500 dark:text-zinc-500">No trend data yet</p>
      </div>
    );
  }

  return (
    <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/[0.06] rounded-xl p-5 flex-1 min-w-0">
      <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-1">Collections Trend</h3>
      <p className="text-xs text-gray-500 dark:text-zinc-500 mb-4">Monthly recovery vs total invoiced</p>
      ...
      <div className="flex gap-4 mt-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-gray-500 dark:text-zinc-400">Recovered</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-0.5 bg-indigo-500 rounded inline-block" />
          <span className="text-[11px] text-gray-500 dark:text-zinc-400">Invoiced</span>
        </div>
      </div>
    </div>
  );
