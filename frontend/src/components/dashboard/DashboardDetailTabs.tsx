        ...
        {TABS.map(tab => (
          ...
          >
            ...
            {tab === 'At-Risk Customers' && atRiskList.length > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 bg-rose-500/20 text-rose-400 rounded text-[10px]">
                {atRiskList.length}
              </span>
            )}
          </button>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center">
              <p className="text-xs text-zinc-500">No at-risk customers detected</p>
            </div>
          ) : (
                ...
                {atRiskList.map((c, i) => (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    <td className="px-5 py-3">
                      ...
                      <p className="text-zinc-500 text-[10px]">{c.customerEmail}</p>
                    </td>
                    <td className="px-5 py-3 text-right font-medium text-white">{fmt(c.totalOwed)}</td>
                    ...
                    <td className="px-5 py-3 text-right text-zinc-400">{c.unpaidInvoices}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                  ...
                  return (
                    <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                      <td className="px-5 py-3">
                        ...
                        <p className="text-zinc-500 text-[10px]">{b.days}</p>
                      </td>
                      <td className="px-5 py-3 text-right text-zinc-300">{b.invoiceCount}</td>
                      <td className="px-5 py-3 text-right font-medium text-white">{fmt(b.amount)}</td>
                      ...
                      <td className="px-5 py-3">
                        {i === 0 ? (
                          ...
                          <span className="text-[10px] text-rose-400 font-medium">⚠ Write-off risk</span>
                        ) : (
                          <span className="text-[10px] text-amber-400">Action needed</span>
                        )}
                      </td>
                    </tr>
                  );
