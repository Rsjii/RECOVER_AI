...
function FunnelBar({ stage, maxValue }: { stage: FunnelStage; maxValue: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-[11px] text-zinc-400 w-24 flex-shrink-0 text-right leading-tight">
        {stage.label}
      </span>
...
export default function RecoveryFunnelChart({
        ...
        <div className="space-y-3">
          {[100, 82, 31, 10, 5].map((w, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="h-6 bg-white/10 rounded" style={{ width: `${w}%` }} />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-5">
      <h3 className="text-sm font-semibold text-white mb-1">Recovery Funnel</h3>
      <p className="text-xs text-zinc-500 mb-4">At-risk → Paid conversion</p>
      ...
      <div className="mt-4 pt-3 border-t border-white/[0.05] flex justify-between text-[11px]">
        <span className="text-zinc-500">End-to-end recovery</span>
