...
const getSidebarSections = (isAdmin: boolean): SidebarSection[] => [
  ...
  },
  {
    ...
    items: [
      ...
      // ❌ REMOVED: Email Queue moved to Activity → Emails tab
      // {
      //   path: '/email-queue',
      //   label: 'Email Queue',
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* ════════════════════════════════════════════════════════════════ */}
      {/* ALERT CENTER — Floating Popover Design (Premium 2026 Pattern) */}
      {/* ════════════════════════════════════════════════════════════════ */}
      {(() => {
        const hasStripeAlert = !company?.stripe_account_id;
        const hasEmailAlert = company && !company.smtp_verified;
        const alertCount = (hasStripeAlert ? 1 : 0) + (hasEmailAlert ? 1 : 0);

        if (alertCount === 0) return null;

        return (
          <>
            {/* Alert Button — Always visible (expanded or collapsed) */}
            <div className="border-t border-gray-200 dark:border-white/[0.06] shrink-0">
              <div className={cn('space-y-1', collapsed ? 'px-2' : 'px-3')}>
                <button
                  ...
                  title={collapsed ? `${alertCount} alert${alertCount !== 1 ? 's' : ''}` : undefined}
                >
                {/* Alert Icon (Bell) with Subtle Red Dot */}
                <div className="relative flex-shrink-0 text-gray-500 dark:text-gray-500">
                  ...
                  <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-600 dark:bg-red-500 rounded-full" />
                </div>
            ...
            {/* Floating Popover — Premium Design */}
            {alertsPopoverOpen && (
              <>
                ...
                {/* Popover Card */}
                <div className="fixed z-50 bg-white dark:bg-[#1a1a1c] border border-gray-200 dark:border-white/[0.08] rounded-xl shadow-xl overflow-hidden"
                  style={{
                    top: '80px',
                    left: collapsed ? '76px' : '268px',
                    width: '320px',
                ...
                >
                  {/* Popover Header */}
                  <div className="sticky top-0 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 px-4 py-3 border-b border-gray-200 dark:border-white/[0.08] flex items-center justify-between">
                  ...
                  <div className="space-y-2 p-3">
                    {/* Stripe Alert */}
                    {hasStripeAlert && (
                      <div className="p-3 bg-red-50 dark:bg-red-900/15 border border-red-200 dark:border-red-800/40 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/25 transition-colors">
                        <div className="flex items-start gap-2.5">
                          <span className="text-lg mt-0.5">🔴</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-red-800 dark:text-red-300">Stripe Not Connected</p>
                            <p className="text-xs text-red-700 dark:text-red-400 mt-0.5">Agent paused. Reconnect to resume.</p>
                            <button
                              onClick={() => {
                                navigate('/settings?tab=integrations');
                                setAlertsPopoverOpen(false);
                              }}
                              className="text-xs font-medium text-red-700 dark:text-red-300 hover:text-red-900 dark:hover:text-red-200 underline mt-1.5 block transition-colors"
                            >
                              Connect Stripe →
                            </button>
                          </div>
                    ...
                    {/* Email Alert */}
                    {hasEmailAlert && (
                      <div className="p-3 bg-blue-50 dark:bg-blue-900/15 border border-blue-200 dark:border-blue-800/40 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/25 transition-colors">
                        <div className="flex items-start gap-2.5">
                          <span className="text-lg mt-0.5">ℹ️</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-blue-800 dark:text-blue-300">Email Configuration</p>
                            <p className="text-xs text-blue-700 dark:text-blue-400 mt-0.5">Emails via RecoverAI domain.</p>
                            <button
                              onClick={() => {
                                navigate('/settings?tab=email');
                                setAlertsPopoverOpen(false);
                              }}
                              className="text-xs font-medium text-blue-700 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-200 underline mt-1.5 block transition-colors"
                            >
                              Setup custom →
                            </button>
                          </div>
