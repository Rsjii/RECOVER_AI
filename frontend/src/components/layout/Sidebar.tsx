...
const getSidebarSections = (isAdmin: boolean, isTrial: boolean): SidebarSection[] => {
  const sections: SidebarSection[] = [
    {
      ...
      items: [
        ...
        // Hide Customers tab for trial users (AR recovery feature)
        ...(isTrial ? [] : [
          {
            path: '/customers',
            label: 'Customers',
  ...
  // Hide entire OPERATIONS section for trial users
  if (!isTrial) {
    sections.push({
      title: 'OPERATIONS',
      items: [
...
export const Sidebar: React.FC = () => {
  ...
  const isTrial = trial?.status === 'trialing';
  const sections = getSidebarSections(isAdmin, isTrial);
