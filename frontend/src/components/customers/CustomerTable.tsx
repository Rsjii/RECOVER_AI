...
type SortKey = 'total_ar_balance' | 'customer_risk_score' | 'unpaid_invoice_count' | null;
            ...
            return (
              <div key={c.id} onClick={() => onRowClick(c)}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{c.unpaid_invoice_count ?? 0} unpaid</span>
                </div>
          ...
          <thead className="bg-gray-50 dark:bg-white/[0.03] border-b border-gray-200 dark:border-white/[0.08]">
            <tr>
              ...
              <th className="px-6 py-3 font-semibold text-gray-900 dark:text-white cursor-pointer select-none hover:text-indigo-600 dark:hover:text-indigo-400"
                ...
                Unpaid Inv <SortIcon active={sortKey === 'unpaid_invoice_count'} dir={sortDir} />
              </th>
              <th className="px-6 py-3 font-semibold text-gray-900 dark:text-white cursor-pointer select-none hover:text-indigo-600 dark:hover:text-indigo-400"
            ...
            {sorted.map(c => {
              ...
              return (
                <tr key={c.id}
                  ...
                  <td className="px-6 py-4 cursor-pointer" onClick={() => onRowClick(c)}>
                    {c.payment_insights?.reliability_pct != null ? (
                      <div className="flex items-center gap-2 text-xs">
                        {c.payment_insights.reliability_pct >= 80 ? (
                          <span>🟢 Reliable</span>
                        ) : c.payment_insights.reliability_pct >= 50 ? (
                          <span>🟠 Slow{c.payment_insights.avg_days_to_pay ? ` (+${c.payment_insights.avg_days_to_pay}d)` : ''}</span>
                        ) : (
                  ...
                  <td className="px-6 py-4 text-center cursor-pointer" onClick={() => onRowClick(c)}>
                    <span className="font-medium text-gray-900 dark:text-white">{c.unpaid_invoice_count ?? 0}</span>
                  </td>
